...
const landingPage: HTMLDivElement = document.querySelector(
  ".page.landing"
) as HTMLDivElement;
const pageOne: HTMLDivElement = document.querySelector(
  ".page.one"
) as HTMLDivElement;
const pageTwo: HTMLDivElement = document.querySelector(
  ".page.two"
) as HTMLDivElement;
const pageThree: HTMLDivElement = document.querySelector(
  ".page.three"
) as HTMLDivElement;
const pageFour: HTMLDivElement = document.querySelector(
  ".page.four"
) as HTMLDivElement;

const nextButtonLandingPage: HTMLDivElement = document.querySelector(
  ".next-button#page-landing-next"
) as HTMLDivElement;
const nextButtonPageOne: HTMLDivElement = document.querySelector(
  ".next-button#page-one-next"
) as HTMLDivElement;
const nextButtonPageTwo: HTMLDivElement = document.querySelector(
  ".next-button#page-two-next"
) as HTMLDivElement;

const reshapeButton: HTMLDivElement = document.querySelector(
  ".button.shape"
) as HTMLDivElement;
const shareButton: HTMLDivElement = document.querySelector(
  ".button.share"
) as HTMLDivElement;

const refinementContainer: HTMLDivElement =
  document.querySelector(".radial-interface")!;
...
nextButtonLandingPage.addEventListener("click", () => {
  ...
  cardChoiceInterface.setActiveCard(
    cardChoiceInterface.activeCard.cardRepresentation
  );
...
nextButtonPageOne.addEventListener("click", () => {
  ...
  setTimeout(() => {
    shapePreviewInterface = new RadialPreview(prompt, refinementContainer);
  }, 1000);
...
nextButtonPageTwo.addEventListener("click", () => {
  ...
  let traitMeters: HTMLDivElement = document.querySelector(
    ".page.three .parameters-block"
  ) as HTMLDivElement;
  let traitClouds: NodeList = document.querySelectorAll(
    ".page.three .shape-avatar-preview .card-backgrounds div"
  );

  let agentTitle: HTMLSpanElement = document.querySelector(
    ".page.three .page-title span.job"
  ) as HTMLSpanElement;
  let agentCode: HTMLSpanElement = document.querySelector(
    ".page.three .page-title span.trait-code"
  ) as HTMLSpanElement;

  let traits: string = "";
  let icon: HTMLDivElement = document.querySelector(
  ...
  Object.keys(shapePreviewInterface.parameters).forEach((key) => {
    ...
    let meter: HTMLDivElement = traitMeters.querySelector(
      `.trait.${key.toLowerCase()}`
    ) as HTMLDivElement;
    if (meter) {
      meter.classList.add("selected");
      let progressBar: HTMLDivElement = meter.querySelector(
        ".progress-bar"
      ) as HTMLDivElement;
      let color: string;
  ...
  const shapeContainer: HTMLDivElement = document.querySelector(
    ".shape-avatar-preview .constructed-shape"
  ) as HTMLDivElement;
  const shapeGenerator: ShapeGenerator = new ShapeGenerator(
    shapeContainer,
    shapePreviewInterface.parameters
  );
...
function assignBackgroundColors(card: Card) {
  fullPageGradients.container.classList.add(
    card.cardRepresentation.id.toLowerCase(),
    "active"
  );
...
function parseQuery(query: string) {
  ...
  if (chat > 0 && agentQueryCode <= 3) {
    ...
    const landingMessage: HTMLDivElement = document.querySelector(
    ...
    Object.keys(queryParameters).forEach((key) => {
      ...
      if (queryParameters[parameter] > 0) {
        let parameterValue = Math.floor(
          (queryParameters[parameter] / 15) * 0.5
        );
    ...
    let request = queryPrompt.makeLLMRequest("user", promptMessage).then(() => {
      ...
      const screenContainer: HTMLDivElement = document.querySelector(
        ".screen"
      ) as HTMLDivElement;
    ...
    switch (agentQueryCode) {
      ...
      case 3:
        ...
        newCard = document.querySelector(
          ".card#Productivity"
        ) as HTMLDivElement;
        break;
      default:
...
shareButton.addEventListener("click", () => {
  window.open(
    `http://emotive-ai.ustwo.com/sharecard/${makeSharedURLQuery()}`,
    "_blank"
  );
});
...
function makeSharedURLQuery(refer: boolean = true): string {
  ...
  if (shapePreviewInterface.parameters.assertive > 10) {
    sharedURLQuery += `as=${shapePreviewInterface.parameters.assertive}&`;
  }
  if (shapePreviewInterface.parameters.compassionate > 10) {
    sharedURLQuery += `co=${shapePreviewInterface.parameters.compassionate}&`;
  }
  if (shapePreviewInterface.parameters.curious > 10) {
    sharedURLQuery += `cu=${shapePreviewInterface.parameters.curious}&`;
  }
  if (shapePreviewInterface.parameters.excited > 10) {
    sharedURLQuery += `ex=${shapePreviewInterface.parameters.excited}&`;
  }
  if (shapePreviewInterface.parameters.optimistic > 10) {
    sharedURLQuery += `op=${shapePreviewInterface.parameters.optimistic}&`;
  }
  if (shapePreviewInterface.parameters.playful > 10) {
    sharedURLQuery += `pl=${shapePreviewInterface.parameters.playful}&`;
  }
